...
{
	LoadState(vInterface);
	return (DHCPClient.smState != SM_DHCP_DISABLED ? TRUE : FALSE);
}
...
{
	LoadState(vInterface);
	return (DHCPClient.flags.bits.bIsBound ? TRUE : FALSE);
}
...
{
	LoadState(vInterface);
	return (DHCPClient.flags.bits.bDHCPServerDetected ? TRUE : FALSE);
}

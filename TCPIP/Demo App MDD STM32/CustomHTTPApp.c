...
BYTE lastSuccess = FALSE;

// Stick status message variable.  See lastSuccess for details.
BYTE lastFailure = FALSE;
...
HTTP_IO_RESULT HTTPExecutePost(void)
{
	...
	do
	{
		...
		{
			filename[cntr1] = '.';  //File extension starts after this
			cntr1++;
			filext=TRUE;
		}
...
static HTTP_IO_RESULT HTTPPostDDNSConfig(void)
{
	...
	switch(curHTTP.smPost)
	{
		...
		case SM_DDNS_READ_SERVICE:
			...
			DDNSSetService((DDNS_SERVICES)atol((char*)curHTTP.data));

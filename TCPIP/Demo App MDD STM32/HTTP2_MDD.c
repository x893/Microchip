...
static BOOL HTTPSendFile(void)
{
	...
	static DWORD availbleTcpBuffSize;
	//!!! static DWORD cntr = 0;
	static BYTE lock = 0;
	...
	switch(smHTTPSendFile)
	{
		...
		case SM_GET_NO_OF_FILES:
			...
			FileReadUInt32(&recrdcntr,FileRcrdPtr);//Reading Number of files in record
		...
		case SM_GET_HASH_RCRD:
			...
			while(recrdcntr)
			{
				FileReadUInt16(&nameHashRcrd,FileRcrdPtr);//Reading HashName record	
		...
		case SM_GET_DYN_VAR_FILE_RCRD:
			...
			if (lock==0)
			{
				FileReadUInt32(&UInt32DataFromBinFile,DynVarRcrdFilePtr);//Reading Record Length 
				FileReadUInt16(&UInt16DataFromBinFile,DynVarRcrdFilePtr);//Reading Flags
				lock=1;
			}
			
			FileReadUInt32(&dynVarRcrdOffset,DynVarRcrdFilePtr);//Reading dynamic variable offset in webpage			
			FileReadUInt32(&dynVarCallBackID,DynVarRcrdFilePtr);//Reading dynamic variable call back ID	
		...
		case SM_PARSE_TILL_DYN_VAR:
			...
			else
			{
				...
				if (availbleTcpBuffSize >= 64)
				{
					...
					{
						FSfread(sendDataBuffer, 64, 1, curHTTP.file);
					...
					{
						FSfread(sendDataBuffer, (dynVarRcrdOffset-bytesReadCount), 1, curHTTP.file);
				...
				else if (availbleTcpBuffSize != 0)
				{
					if (bytesReadCount != dynVarRcrdOffset )
					{
						...
						{
							FSfread(sendDataBuffer, availbleTcpBuffSize, 1, curHTTP.file);
		...
		case SM_PARSE_DYN_VAR_STRING:
			...
			FSfread(sendDataBuffer, 1, 1, curHTTP.file);
			numBytes-=1;
			bytesReadCount+=1;
			if (sendDataBuffer[0]=='~')
			{
		...
		case SM_SERVE_TEXT_DATA:
			...
			if (availbleTcpBuffSize >= 64)
			{
				...
				{
					FSfread(sendDataBuffer, numBytes, 1, curHTTP.file);
...
void HTTPIncFile(ROM BYTE* cFile)
{
	...
	FILE_HANDLE fp;
	static DWORD availbleTcpBuffSize, numBytes;
	static BYTE lock = 0;
	...
	if (availbleTcpBuffSize >= 64)
	{
		if (numBytes <= 64)
		{
			FSfread(incDataBuffer, numBytes, 1, fp);
			TCPPutArray(sktHTTP, incDataBuffer, numBytes);
			numBytes = 0;
		}	
		else
		{	
			FSfread(incDataBuffer, 64, 1, fp);
	...
	{	
		...
		{
			FSfread(incDataBuffer, availbleTcpBuffSize, 1, fp);
